...
function renderDetailViews(total,subs){
 ...
 const expenses=$('#allExpensesList');expenses.innerHTML=state.expenses.length?`<div class="data-list-head"><span>Despesa</span><span>Valor</span><span>Vencimento</span><span>Status</span><span>Ações</span></div>${state.expenses.map(e=>`<div class="data-list-row"><div><strong>${e.name}</strong><small>${e.type==='recurring'?'Recorrente':'Única'}</small></div><strong>${money(e.value)}</strong><span>${shortDate(e.due)}</span><span class="status-pill ${e.status==='paid'?'paid':''}">${e.status==='paid'?'Pago':'Pendente'}</span><div class="table-actions"><button title="Marcar como paga" data-pay="${e.id}">✓</button><button title="Editar" data-edit="${e.id}">✎</button><button title="Duplicar" data-copy="${e.id}">⧉</button><button title="Excluir" data-delete="${e.id}">×</button></div></div>`).join('')}`:`<div class="view-empty"><div class="empty-icon">↗</div><h3>Nenhuma despesa cadastrada</h3><p>Use “Nova despesa” para criar seu primeiro registro.</p></div>`;
