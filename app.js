...
let state;
try{state={...structuredClone(defaults),...JSON.parse(localStorage.getItem('equilibra-data')||'null')}}catch{state=structuredClone(defaults);localStorage.removeItem('equilibra-data')}
...
function openDrawer(html){$('#drawerContent').innerHTML=html;$('#overlay').classList.add('open');$('#drawer').classList.add('open');$('#drawer').setAttribute('aria-hidden','false')}
...
function renderDetailViews(total,subs){
 ...
 const subscriptions=$('#allSubscriptionsList');subscriptions.innerHTML=state.subscriptions.length?`<div class="data-list-head"><span>Assinatura</span><span>Valor</span><span>Próxima cobrança</span><span>Status</span><span>Ações</span></div>${state.subscriptions.map(s=>{const service=s.serviceId?catalogService(s.serviceId):guessService(s.name),mark=s.mark||service.mark,color=s.color||service.color;return `<div class="data-list-row"><div class="subscription-identity"><span class="service-mark" style="--service-color:${color}">${mark}</span><span><strong>${escapeHtml(s.name)}</strong><small>${s.planName?`${escapeHtml(s.planName)} · `:''}${escapeHtml(s.period)}</small></span></div><strong>${money(s.value)}</strong><span>${shortDate(s.next)}</span><span class="status-pill ${s.active?'active':'inactive'}">${s.active?'Ativa':'Inativa'}</span><div class="table-actions"><button title="Ativar ou desativar" data-toggle-sub="${s.id}">${s.active?'Ⅱ':'▶'}</button><button title="Editar" data-edit-sub="${s.id}">✎</button><button title="Excluir" data-delete-sub="${s.id}">×</button></div></div>`}).join('')}`:`<div class="view-empty"><div class="empty-icon">◌</div><h3>Nenhuma assinatura cadastrada</h3><p>Use “Nova assinatura” para começar a acompanhar cobranças.</p></div>`;
 $$('[data-toggle-sub]').forEach(b=>b.onclick=()=>{const s=state.subscriptions.find(x=>x.id===b.dataset.toggleSub);s.active=!s.active;save();toast(s.active?'Assinatura ativada':'Assinatura pausada')});$$('[data-edit-sub]').forEach(b=>b.onclick=()=>openSubscription(state.subscriptions.find(x=>x.id===b.dataset.editSub)));$$('[data-delete-sub]').forEach(b=>b.onclick=()=>{state.subscriptions=state.subscriptions.filter(x=>x.id!==b.dataset.deleteSub);save();toast('Assinatura excluída')});
}
